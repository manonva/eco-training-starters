...
function CommerceHeader({ cartCount }: Readonly<{ cartCount: number }>) {
  return (
...
function ProductDetailPage({ products }: Readonly<{ products: Product[] }>) {
  const { id } = useParams();
...
function CartPage({ cart }: Readonly<{ cart: CartPayload }>) {
  return (
